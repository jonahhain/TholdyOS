...
build $image="tholdyos" $tag="stable" $flavor="main" rechunk="0" ghcr="0" pipeline="0" $kernel_pin="":
    ...
    # Try to list existing tags, but don't fail if repo doesn't exist yet (first build)
    if skopeo list-tags docker://ghcr.io/{{ repo_organization }}/${image_name} > /tmp/repotags.json 2>/dev/null; then
...
rechunk $image="tholdyos" $tag="stable" $flavor="main" ghcr="0" pipeline="0":
    ...
    ${SUDOIF} ${PODMAN} run --rm \
        ...
        --volume cache_ostree:/var/ostree \
        --env REPO=/var/ostree/repo \
        --env PREV_REF=ghcr.io/jonahhain/"${image_name}":"${tag}" \
